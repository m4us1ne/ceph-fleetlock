...
from cephadm.services.cephadmservice import CephService

_LOCK_PREFIX = "lock/"


class cephlock_servie(CephService):
    TYPE = "cephlock"
...
class FleetLockAPI:
    ...
    def check_time_slots(self):
        try:
            ...
            before = datetime.time.fromisoformat(
                self.module.read_kv("UpdateOnlyBefore")
            )
        except ValueError:
            # log parsing error
            return False
...
class Module(MgrModule):
    ...
    def cli_set_config(
        ...
        if not after == "invalid":
            try:
                cmd["UpdateOnlyAfter"] = datetime.time.fromisoformat(after).strftime(
                    "%H:%M:%S"
                )
            except ValueError:
        ...
        if not before == "invalid":
            try:
                cmd["UpdateOnlyBefore"] = datetime.time.fromisoformat(before).strftime(
                    "%H:%M:%S"
                )
            except ValueError:
